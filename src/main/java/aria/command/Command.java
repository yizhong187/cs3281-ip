...
public class Command {
     ...
     * @throws AriaException if the command cannot be executed
     */
    public void execute(TaskList taskList, Ui ui, Storage storage) throws AriaException {
        switch (type) {
        case LIST:
        ...
        case TAG: {
            ...
            if (tagParts.length < 2) {
                throw new AriaException("OOPS!!! Please provide a tag name.");
            }
        ...
        case UNTAG: {
            ...
            if (untagParts.length < 2) {
                throw new AriaException("OOPS!!! Please provide a tag name to remove.");
            }
        ...
        case SCHEDULE:
            ui.showFoundTasks(taskList.getScheduleOn(
                    aria.util.DateParser.parse(description) != null
                    ? aria.util.DateParser.parse(description)
     ...
     * @throws AriaException if the command cannot be executed
     */
    public String getOutput(TaskList taskList, Storage storage) throws AriaException {
        switch (type) {
        case LIST: {
            if (taskList.size() == 0) {
        ...
        case FREETIME: {
            ...
            java.time.LocalDate freeDate = aria.util.DateParser.parse(freeParts[0]);
            if (freeDate == null) {
        ...
        case FIXED: {
            ...
            try {
                ...
                throw new AriaException("OOPS!!! Duration must be a number (e.g. 1.5 for 1.5 hours).");
            }
            boolean fixedDup = taskList.hasDuplicate(description);
            Task fixedTask = new aria.task.FixedDurationTask(description, hours);
        ...
        case WITHIN: {
            ...
            Task task = new aria.task.WithinPeriodTask(description, from, to);
        ...
        case TEVENT: {
            ...
            if (slots.isEmpty()) {
                throw new AriaException("OOPS!!! Provide at least one slot: /slot1 <time>");
            }
            aria.task.TentativeEvent te = new aria.task.TentativeEvent(teDesc, slots);
        ...
        case CONFIRM: {
            ...
            if (!(task instanceof aria.task.TentativeEvent)) {
                throw new AriaException("OOPS!!! Task " + (idx + 1) + " is not a tentative event.");
            }
            aria.task.TentativeEvent te = (aria.task.TentativeEvent) task;
            int slotNum;
            try {
                slotNum = Integer.parseInt(cp.length > 1 ? cp[1].trim() : "") - 1;
            } catch (NumberFormatException e) {
                throw new AriaException("OOPS!!! Please provide a valid slot number.");
            }
            if (slotNum < 0 || slotNum >= te.getSlots().size()) {
                throw new AriaException("OOPS!!! Slot " + (slotNum + 1) + " does not exist.");
            }
        ...
        case STATS: {
            ...
            long todos = IntStream.range(0, taskList.size())
                    .filter(i -> taskList.get(i) instanceof aria.task.Todo).count();
        ...
        case SNOOZE: {
            ...
            if (task instanceof Deadline) {
                ...
                throw new AriaException("OOPS!!! Only deadlines and events can be snoozed.");
            }
            storage.save(taskList);
            return "Snoozed task:\n  " + taskList.get(index);
        }
        case SCHEDULE: {
            java.time.LocalDate date = aria.util.DateParser.parse(description);
            if (date == null) {
        ...
        case EXPENSE:
            return handleEntity("./data/expenses.txt", description,
                    new String[]{"Desc", "Amount", "Category"},
                    (store, args) -> {
                        String desc = args.replaceAll("/amount.*", "").replaceAll("/category.*", "").trim();
                        store.add(desc, extractField(args, "amount"), extractField(args, "category"));
                    },
        ...
        case PLACE:
            return handleEntity("./data/places.txt", description,
                    new String[]{"Name", "Review", "Rating"},
                    (store, args) -> {
                        String name = args.replaceAll("/review.*", "").replaceAll("/rating.*", "").trim();
                        store.add(name, extractField(args, "review"), extractField(args, "rating"));
                    },
    ...
    private interface EntityAdder {
        void add(aria.entity.SimpleEntityStore store, String args) throws AriaException;
    }
    ...
    private String handleEntity(String storePath, String desc, String[] fieldNames,
            EntityAdder adder, EntryFormatter formatter, String usage) throws AriaException {
        aria.entity.SimpleEntityStore store = new aria.entity.SimpleEntityStore(storePath);
        ...
        case "add":
            if (args.isEmpty()) {
                throw new AriaException("OOPS!!! Usage: " + usage);
            }
        ...
        case "delete": {
            ...
            try {
                ...
                throw new AriaException("OOPS!!! Please provide a valid index.");
            }
            if (idx < 0 || idx >= store.size()) {
                throw new AriaException("OOPS!!! Entry " + (idx + 1) + " does not exist.");
            }
    ...
    private String handleTrivia(String desc) throws AriaException {
        aria.entity.SimpleEntityStore store = new aria.entity.SimpleEntityStore("./data/trivia.txt");
        String[] parts = desc.split("\\s+", 2);
        String sub = parts[0].toLowerCase();
        String args = parts.length > 1 ? parts[1] : "";
        switch (sub) {
        case "add": {
            ...
            if (question.isEmpty() || answer.isEmpty()) {
                throw new AriaException("OOPS!!! Usage: trivia add QUESTION /answer ANS");
            }
     ...
     * @throws AriaException if any token is invalid or out of range
     */
                ...
                try {
                    ...
                    for (int i = lo; i <= hi; i++) {
                        if (i < 0 || i >= taskList.size()) {
                            throw new AriaException("OOPS!!! Task " + (i + 1) + " does not exist.");
                        }
                        indices.add(i);
                    }
                } catch (NumberFormatException e) {
                    throw new AriaException("OOPS!!! Invalid range: " + token);
                }
            } else {
     ...
     * @throws AriaException if the index is not a valid number or out of range
     */
    private int parseIndex(String indexStr, TaskList taskList) throws AriaException {
        try {
            int index = Integer.parseInt(indexStr) - 1;
            if (index < 0 || index >= taskList.size()) {
                throw new AriaException("OOPS!!! Task number " + (index + 1) + " does not exist.");
            }
            return index;
        } catch (NumberFormatException e) {
            throw new AriaException("OOPS!!! Please provide a valid task number.");
        }
    }
    ...
    private void applyUpdates(TaskList taskList, int index, String args) {
        ...
        if (priorityMatcher.find()) {
            task.setPriority(aria.task.Priority.fromString(priorityMatcher.group(1)));
        }
        if (task instanceof Deadline) {
